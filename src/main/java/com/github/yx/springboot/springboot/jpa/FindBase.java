package com.github.yx.springboot.springboot.jpa;
...
public abstract class FindBase<T extends BaseModel,ID extends Serializable>  implements IFindBase<T,ID>{
	 ...
	 * 自定义查询条件
	 */
	public abstract void where(T t,List<Predicate>  predicates,Root<T> root, CriteriaQuery<?> query, CriteriaBuilder cb,String... other);
	...
	public Page<T> findAll(T t, PageRequest pageRequest,TimeEntity createTimeEntity,TimeEntity updTimeEntity,String... other) {		
		Page<T> page =  specjpa().findAll(getSpecification(t,createTimeEntity,updTimeEntity,other),pageRequest);
		setSelect(page);
		return page;
	}
	
	public T find(T t,TimeEntity createTimeEntity,TimeEntity updTimeEntity,String... other) {
		List<T> list = specjpa().findAll(getSpecification(t,createTimeEntity,updTimeEntity,other));
...
//				if(obj!=null)
	 ...
	 * @return
	 */
	private  Specification<T> getSpecification(T t,TimeEntity cretTimeEntity,TimeEntity updTimeEntity,String...other){
			...
			public Predicate toPredicate(Root<T> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if(t!=null)
					where(t,predicates,root,query,cb,other);
	...
	private List<Predicate> getPredicates(Root<T> root, CriteriaQuery<?> query, CriteriaBuilder cb,TimeEntity createTimeEntity,TimeEntity updTimeEntity) {
		...
		//createTimeb
		if(createTimeEntity!=null && createTimeEntity.begainTime !=null ) {
			predicates.add(cb.greaterThanOrEqualTo(root.get("creTime"), new Date(createTimeEntity.begainTime)));
		}
		//createTimee
		if(createTimeEntity!=null && createTimeEntity.endTime !=null) {
			predicates.add(cb.lessThanOrEqualTo(root.get("creTime"), new Date(createTimeEntity.endTime)));
		}
		//updTimeb
		if(updTimeEntity!=null && updTimeEntity.begainTime !=null ) {
			predicates.add(cb.greaterThanOrEqualTo(root.get("updTime"), new Date(updTimeEntity.begainTime)));
		}
		//updTimee
		if(updTimeEntity!=null && updTimeEntity.endTime !=null) {
			predicates.add(cb.lessThanOrEqualTo(root.get("updTime"), new Date(updTimeEntity.endTime)));
		}
		//删除标识
